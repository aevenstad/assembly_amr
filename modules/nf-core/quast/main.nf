process QUAST {
    publishDir "${params.outdir}/${meta.id}/quast", mode: 'copy'
    tag "${meta.id}"
    ...
    output:
    path "*", emit: results
    tuple val(meta), path("${meta.id}_report.tsv"), emit: tsv
    ...
    # ln -s report.tsv ${prefix}.tsv
    [ -f contigs_reports/all_alignments_transcriptome.tsv ] && ln -s 4_quast/contigs_reports/all_alignments_transcriptome.tsv ${prefix}_transcriptome.tsv
    [ -f contigs_reports/misassemblies_report.tsv ] && ln -s 4_quast/contigs_reports/misassemblies_report.tsv ${prefix}_misassemblies.tsv
    [ -f contigs_reports/unaligned_report.tsv ] && ln -s 4_quast/contigs_reports/unaligned_report.tsv ${prefix}_unaligned.tsv
